for k = 1:100
    ...
    source = strcat('.\predict_face\training_leaves\',num2str(k),'.jpg');
    img=imread(source);
    v=zeros(480,640);
    R=img(:,:,1);
    G=img(:,:,2);
    ...
    %%-----------------------------------
    name=strcat('.\predict_face\training_leaves\after_extraction\',num2str(k),'.jpg');
    %%----------------------------------------
    imwrite(b,name);
   %figure(1),imshow(img);
    pp=rgb2gray(img);
...
%%----------------------------------------------------------------------
xlswrite('.\predict_face\training\training.xlsx',arr);
%%------------------------------------------------------------